...
def make_prompt_weekly(data: dict) -> str:
    ...
    # 전년 거래액이 0이거나 없을 경우 ZeroDivisionError 방지
    prev_rev = kpi_y.get("revenue") or 1
    return f"""
LF몰 제휴 {t7.get('wk_cur','')} 주차 실적입니다.
 
[주차 KPI]
- 거래액: {kpi.get('revenue',0)/1e6:.1f}M (전년동주: {kpi_y.get('revenue',0)/1e6:.1f}M, YoY: {(kpi.get('revenue',0)/prev_rev-1)*100:+.1f}%)
